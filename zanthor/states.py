...
class GameOver(engine.State):
    ...
    def event(self, e):
        if e.type == KEYDOWN and e.key == K_RETURN:  # or e.type == MOUSEBUTTONDOWN:
...
class NextLevel(engine.State):
    ...
    def event(self, e):
        ...
        if e.type == KEYDOWN and e.key in (K_1, K_2, K_3, K_4, K_5, K_6, K_7, K_8):
            key = e.key - K_1
...
class GameWon(engine.State):
    ...
    def paint(self, screen):
        ...
        x, y = 440 - img.get_width() / 2 + random.randrange(
            -24, 24
        ), 240 - img.get_height() / 2 + random.randrange(-12, 12)
        screen.blit(img, (x, y))
